...
* **多源数据导入：** 支持手动创建、Excel/CSV 批量导入、JSON 方案导入（兼容历史格式与 schema: `heat-treatment-digital-twin-v1`）。
* **智能化装炉算法矩阵（已实现/运行中）：**
  - **异构空间填充 (Heterogeneous):** 基于空位分割的贪心放置，适用于不分层的快速装炉场景。
  - **搁板分层平铺 (Shelf-Layered):** 自底向上分层平铺，动态计算搁板高度与厚度，支持多层搁板输出。
  - **重心居中 (Center-of-Gravity):** 中心向外扩散候选放置 + XZ 面重心打分，Y 轴由重力/搁板控制（Y 不参与重心打分）。
  - **统一嵌套算法 (Shelf+CG Unified):** 外层由搁板驱动 Y 分层，内层在每一层用重心居中算法求 X/Z 最优落子（已作为默认策略在开启搁板或重心时使用）。
  - **姿态优化与圆盘翻转：** 自动最小面积面朝下、圆盘翻转阈值可配置（discFlipRatio）。
  - **工装/坐标系特殊处理：** 支持环形工装（polar/ring-tooling）与夹具、挂具的专属几何与避障规则。
* **分组与聚集规则：** 支持按工艺/材质分组（sameProcess / sameMaterial），分组后对每组独立执行装炉，产生聚集率统计与分组摘要。
* **装炉策略开关（策略驱动评分权重）：**
  - 重心稳定 (`balanced`)
  - 空间利用率优先 (`spaceUtil`)
  - 热场均衡装载 (`thermalBalance`)
  - 表面均匀性优先 (`surfaceUniform`)
* **约束透传与工装物理语义：** 每个炉膛可自带 toolingType、hasShelf、canStackInside、centerVoidRadius、placementMode 等物理约束，算法在多炉、多工装场景中透传并生效。
...
* **三页式标准化输出：** 包含方案总览、多视图图纸、分层步骤图、AI 综合效益预测报告。
...
* **UI/UX 迭代（已落地主要交互）：** 3D 悬浮座舱、AI 推荐水平信息条、炉次缩略栏、交互式规则面板与导入预览等一系列交互已实现并与渲染层解耦。
* **工装模型库扩展（已实现）：** `basket-model.js` 与 `furnaceTooling` 支持五大工装类型及其专属参数（ring-tooling、special-jig、hanger、material-tray、standard-basket），并可在炉膛卡片/详情中配置 extras 参数。
* **综合效益预测引擎（已集成）：** 在 executePacking 完成后调用预测模块，生成电耗、氮气消耗与质量风险等预测数据，并在 UI 的 AI Summary / 方案分析中显示（含综合评分、热场均匀性、交期满足率等指标）。
* **算法成熟度：** 实现并稳定运行：分组预处理 → 统一嵌套算法（搁板 + 重心）→ 多炉循环 → 聚集率统计 → 预测输出。已加入多项工程化策略权重与特殊规则以满足生产线约束。
